...
function Game(props) {
  ...
  const [timer, setTime] = useState(3); //props.timer
  ...
  useEffect(() => {
    if (props.host) {
      // need to be host so it stays consistent per game and not per person
  ...
  // emit timer -emit
  useEffect(() => {
    socket.on("showendmodal", () => {
      setStartTimer(false);
      // may need a timeout to wait out
      setTimeout(() => {
        setRound(round);
        setRoundEnd(true);
        setShowOverlay(true);
